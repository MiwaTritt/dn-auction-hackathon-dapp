...
const source = fs.readFileSync(campaignPath, 'utf-8'); // path and encoding
const output = solc.compile(source, 1).contracts;

fs.ensureDirSync(buildPath); // will check if dir exists if not creates it

for(let contract in output) { // loop through all contracts
    fs.outputJsonSync( // write out each contract to JSON file to buildPath
